...
${chalk.hex('#FE0041').bold('│')} ❄️ ${chalk.yellowBright('Fecha:')} ${chalk.blueBright(new Date(m.messageTimestamp ? 1000 * (m.messageTimestamp.low || m.messageTimestamp) : Date.now()).toLocaleDateString("es-ES", { timeZone: "America/Mexico_City", day: 'numeric', month: 'long', year: 'numeric' }))}
...
${chalk.hex('#FE0041').bold('│')} ⚖️ ${chalk.magentaBright('Peso del mensaje:')} ${chalk.yellowBright(filesize + ' B')} [${chalk.cyanBright(filesize === 0 ? 0 : (filesize / 1000 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1))} ${chalk.greenBright(['B', 'KB', 'MB', 'GB', 'TB'][Math.floor(Math.log(filesize) / Math.log(1000))] || '')}]
