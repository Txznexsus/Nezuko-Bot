...
let handler = async (m, { conn, usedPrefix, command }) => {
  try {
    ...
    if (!/audio|video/.test(mime) && !/audioMessage|videoMessage/.test(mtype)) {
      return conn.reply(
        m.chat,
        `🎧 *Usa el comando así:*\n\nEtiqueta un audio o video corto con: *${usedPrefix + command}* para reconocer la canción.`,
        m
      )
    }
    ...
    if (!result?.status || result.status.code !== 0)
      throw result?.status?.msg || 'No se pudo identificar la canción.'
...
🩸 “ᴇɴ ʟᴀ ᴏsᴄᴜʀɪᴅᴀᴅ, ʟᴀ ᴍᴜ́sɪᴄᴀ ᴇs ʟᴀ ᴜ́ɴɪᴄᴀ ʟᴜ́ᴢ.” ☯`
              }),
              footer: proto.Message.InteractiveMessage.Footer.fromObject({
                text: 'ShadowCore 🎋'
              }),
      ...
      await conn.reply(m.chat, `🎵 Detectado:\n${title} - ${artist}`, m)
      await m.react('🎶')
    }
