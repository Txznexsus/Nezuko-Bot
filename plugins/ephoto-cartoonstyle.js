...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        const url = `${global.APIs.vreden.url}/api/v1/maker/ephoto/cartoonstyle?text=${encodeURIComponent(text)}`;
...
handler.help = ['cartoonstyle <texto>'];
handler.tags = ['maker'];
handler.command = ['cartoonstyle'];
handler.group = true;
